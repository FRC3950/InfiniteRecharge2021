...
public class GSCPlanBDriveCommand2 extends CommandBase {
  ...
  public GSCPlanBDriveCommand2(DrivetrainSubsystem drivetrainSubsystem, GyroSubsystem gyroSubsystem, IntakeSubsystem intakeSubsystem, BallManipulatorSubsystem ballManipulatorSubsystem) {
  ...
  public void execute() {
    ...
    if(redOrBlue == false){
      ...
      m_ballManipulatorSubsystem.setConveyorMotor(-.8);

      System.out.println("h"+m_gyroSubsystem.getYawAngle());
      System.out.println(count);
      if(m_gyroSubsystem.getYawAngle() > -41 && m_gyroSubsystem.getYawAngle() < -37 && count == 0){
        m_drivetrainSubsystem.motorAngle(0,0);
        finished = true;
      }else {
        m_drivetrainSubsystem.motorAngle(0,.5);
      }
  }
